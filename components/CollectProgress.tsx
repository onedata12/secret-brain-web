...
export default function CollectProgress() {
  const { running, pct, logs, done, query, clear } = useCollect()
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-lg p-4 space-y-2">
        <div className="flex items-center justify-between gap-2">
          <p className="text-xs font-semibold text-slate-700 truncate max-w-[200px]">
            {running ? '🔍 수집 중...' : done?.added ? `✅ ${done.added}개 카드 추가 완료` : '⚠️ 완료 (새 논문 없음)'}
          </p>
          <div className="flex items-center gap-2 shrink-0">
            {done?.added ? (
              <button
                onClick={() => { router.push('/review'); clear() }}
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-slate-400">
            <span className="truncate max-w-[200px]">{lastLog}</span>
            <span className="font-medium text-indigo-500 shrink-0 ml-2">{pct}%</span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-1.5">
            <div
              className={`h-1.5 rounded-full transition-all duration-500 ${done && !running ? 'bg-green-500' : 'bg-indigo-500'}`}
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
